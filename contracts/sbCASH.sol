...
contract StakedButterflyCash is ERC20, Ownable, ReentrancyGuard {
    uint256 constant DECIMALS = 18;
    uint256 constant MAX_CAP = 100000000 * 10**DECIMALS;
    uint256 public cooldown = 1 weeks;

    mapping(address => uint256) public addressToTimestamp;
    ...
    function stake(uint256 _amount) public nonReentrant {
        require(_amount >= 10000 ether, "Minimum stake is 10k $bCASH!");
    ...
    function unStake() public nonReentrant {
        ...
        _bc.transfer(msg.sender, _share);
    ...
    function timeLeftFor(address _staker) public view returns(uint256) {
        if ((addressToTimestamp[_staker] + cooldown) < block.timestamp) {
            return 0;
        } else {
            return (addressToTimestamp[_staker] + cooldown) - block.timestamp;
        }
    }

    function getShareFor(address _staker) public view returns(uint256) {
        uint256 _balance = balanceOf(_staker);
        uint256 _bcBalance = _bc.balanceOf(address(this));
        uint256 _supply = totalSupply();

        if (_supply > 0) {
            return _balance * _bcBalance / totalSupply();
        } else {
            return 0;
        }
    ...
    function totalView() public view returns(uint256, uint256) {
        return (totalSupply(), _bc.balanceOf(address(this)));
    }
